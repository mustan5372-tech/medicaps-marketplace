...
import { FiSend, FiMessageSquare, FiArrowLeft, FiLock } from 'react-icons/fi'
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    socket.on('newMessage', receiveMessage) // legacy
  ...
  useEffect(() => {
    if (!userId) return
    startConversation(userId, listingId).then(conv => {
  ...
  const handleSend = (e) => {
    e.preventDefault()
    if (!text.trim() || !activeConvObj) return
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-4 h-[calc(100vh-72px)]">
      <div className="flex h-full bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
        ...
        <motion.div
          initial={false}
          className={`${mobileShowChat ? 'hidden md:flex' : 'flex'} w-full md:w-80 shrink-0 flex-col border-r border-gray-200 dark:border-gray-800`}
        >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 truncate mt-0.5 flex items-center gap-1">
                        {conv.lastMessage ? <><FiLock className="w-2.5 h-2.5" /> encrypted message</> : 'Start chatting'}
                      </p>
                    </div>
                    ...
                    <>
                      ...
                      {activeConvObj.listing && (
                        <Link to={`/listing/${activeConvObj.listing._id}`}
                          ...
                          {activeConvObj.listing.images?.[0] && (
                            <img src={activeConvObj.listing.images[0]} className="w-6 h-6 rounded object-cover" alt="" />
                          )}
