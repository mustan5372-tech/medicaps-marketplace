      ...
      <Link to={`/listing/${listing._id}`}>
        <div className="relative aspect-[4/3] overflow-hidden bg-gray-100 dark:bg-gray-800">
          ...
          <motion.button onClick={toggleSave} whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.85 }}
            className="absolute top-3 right-3 p-2 rounded-xl bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-200">
            <FiHeart className={`w-4 h-4 transition-colors ${saved ? 'fill-red-500 text-red-500' : 'text-gray-600'}`} />
          </motion.button>
        ...
        <div className="p-4">
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3 className="font-semibold text-gray-900 dark:text-white line-clamp-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors text-sm">
              {listing.title}
            </h3>
          </div>
